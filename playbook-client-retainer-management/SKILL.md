...
description: >
  Generates scope management documentation and scripted protocols for consultants
  and agencies managing ongoing client retainer agreements. Covers scope definition,
  scope creep prevention, change request handling, monthly check-in structure,
  performance review triggers, and retainer renewal. Invoke when setting up a new
  retainer agreement, when a client starts requesting work outside the agreed scope,
  when a retainer is approaching renewal, or when a client relationship is
  deteriorating and needs a structured reset.
---
