...
|---|---|---:|---|
| Audience, consent basis, list condition, offer, sending capability and objectives | Client, approved systems, or dated platform exports | Yes | Stop the affected decision; request it or mark the field unknown and narrow the output. |
...
|---|---|---|
| Email marketing strategy and lifecycle sequence map | Client lead and next workflow owner | Every recommendation traces to an input, names an owner or next action, and marks assumptions and unassessed checks. |
...
If files, platform access, network, rendering, fonts, or calculation tools are unavailable, return the narrowest useful qualified email marketing strategy and lifecycle sequence map. Mark each blocked check `not assessed`, state the consequence, and provide the exact evidence needed to resume. Never convert an unavailable check into a pass.
...
5. Produce the email marketing strategy and lifecycle sequence map, decision/source register and explicit next owner. Do not mutate live systems without separate authority.
...
- Producing the email marketing strategy and lifecycle sequence map without audience. Fix: stop the affected decision or issue a clearly bounded partial output.
...
Given verified audience, the skill produces a email marketing strategy and lifecycle sequence map with source dates and named assumptions. If that evidence cannot be accessed, it returns only the supported sections plus a recovery list; it does not fill gaps with East African defaults.
