...
description: "Use when auditing AI marketing tools for fit, access, cost and governance. Produces AI tool assessment and recommended stack; use `meta-tools-stack-evaluation` when that neighbouring contract is the closer match."
metadata:
  portable: true
  compatible_with:
  - claude-code
...
- Use this skill for auditing AI marketing tools for fit, access, cost and governance.
- Confirm that `meta-tools-stack-evaluation` is not the closer route before proceeding.

## Do Not Use When

- Use `meta-tools-stack-evaluation` when its narrower output is requested.
...
|---|---|---:|---|
| Current tool inventory, use cases and verified prices | Client, approved systems, or dated platform exports | Yes | Stop the affected decision; request it or mark the field unknown and narrow the output. |
...
|---|---|---|
| Ai tool assessment and recommended stack | Client lead and next workflow owner | Every recommendation traces to an input, names an owner or next action, and marks assumptions and unassessed checks. |
...
If files, platform access, network, rendering, fonts, or calculation tools are unavailable, return the narrowest useful qualified AI tool assessment and recommended stack. Mark each blocked check `not assessed`, state the consequence, and provide the exact evidence needed to resume. Never convert an unavailable check into a pass.

## Decision rules

| Choice | Action | Failure or risk avoided |
|---|---|---|
| Current tool inventory, use cases and verified prices is current and attributable | Produce the full AI tool assessment and recommended stack and cite the evidence used. | Decisions based on stale or unrelated evidence. |
...
| The requested outcome belongs to `meta-tools-stack-evaluation` | Route there and hand over the verified inputs already collected. | Neighbour collision and duplicated work. |

## Workflow

1. Confirm the requested decision, consumer, market, period and permission boundary; route to `meta-tools-stack-evaluation` if its contract is closer.
...
5. Produce the AI tool assessment and recommended stack, decision/source register and explicit next owner. Do not mutate live systems without separate authority.
...
- Producing the AI tool assessment and recommended stack without current tool inventory. Fix: stop the affected decision or issue a clearly bounded partial output.
- Treating missing access or data as a successful check. Fix: record `not assessed`, its risk and the recovery input.
- Absorbing `meta-tools-stack-evaluation` into this workflow. Fix: route the neighbouring output and hand over verified inputs.
...
Given verified current tool inventory, the skill produces a AI tool assessment and recommended stack with source dates and named assumptions. If that evidence cannot be accessed, it returns only the supported sections plus a recovery list; it does not fill gaps with East African defaults.

## Read next

- [`meta-tools-stack-evaluation`](../meta-tools-stack-evaluation/SKILL.md) for the neighbouring contract.
