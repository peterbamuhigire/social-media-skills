...
description: "Use when standardising UTM naming and attribution governance for campaigns and dark-social caveats. Produces UTM convention, campaign register and QA checklist; use `meta-analytics-privacy` when that neighbouring contract is the closer match."
metadata:
  portable: true
  compatible_with:
  - claude-code
...
- Use this skill for standardising UTM naming and attribution governance for campaigns and dark-social caveats.
- Confirm that `meta-analytics-privacy` is not the closer route before proceeding.

## Do Not Use When

- Use `meta-analytics-privacy` when its narrower output is requested.
...
|---|---|---:|---|
| Channel taxonomy, campaign names, destination urls and analytics access | Client, approved systems, or dated platform exports | Yes | Stop the affected decision; request it or mark the field unknown and narrow the output. |
...
|---|---|---|
| Utm convention, campaign register and qa checklist | Client lead and next workflow owner | Every recommendation traces to an input, names an owner or next action, and marks assumptions and unassessed checks. |
...
If files, platform access, network, rendering, fonts, or calculation tools are unavailable, return the narrowest useful qualified UTM convention, campaign register and QA checklist. Mark each blocked check `not assessed`, state the consequence, and provide the exact evidence needed to resume. Never convert an unavailable check into a pass.

## Decision rules

| Choice | Action | Failure or risk avoided |
|---|---|---|
| Channel taxonomy, campaign names, destination urls and analytics access is current and attributable | Produce the full UTM convention, campaign register and QA checklist and cite the evidence used. | Decisions based on stale or unrelated evidence. |
...
| The requested outcome belongs to `meta-analytics-privacy` | Route there and hand over the verified inputs already collected. | Neighbour collision and duplicated work. |

## Workflow

1. Confirm the requested decision, consumer, market, period and permission boundary; route to `meta-analytics-privacy` if its contract is closer.
...
5. Produce the UTM convention, campaign register and QA checklist, decision/source register and explicit next owner. Do not mutate live systems without separate authority.
...
- Producing the UTM convention, campaign register and QA checklist without channel taxonomy. Fix: stop the affected decision or issue a clearly bounded partial output.
- Treating missing access or data as a successful check. Fix: record `not assessed`, its risk and the recovery input.
- Absorbing `meta-analytics-privacy` into this workflow. Fix: route the neighbouring output and hand over verified inputs.
...
Given verified channel taxonomy, the skill produces a UTM convention, campaign register and QA checklist with source dates and named assumptions. If that evidence cannot be accessed, it returns only the supported sections plus a recovery list; it does not fill gaps with East African defaults.

## Read next

- [`meta-analytics-privacy`](../meta-analytics-privacy/SKILL.md) for the neighbouring contract.
