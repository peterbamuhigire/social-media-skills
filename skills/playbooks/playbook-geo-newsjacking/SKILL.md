...
description: Use when designing or improving a Geo Newsjacking operating playbook with roles, ordered actions, controls and measures. Use platform skills for channel plans and strategy skills for upstream direction.
metadata:
  portable: true
  compatible_with:
  - claude-code
...
<!-- dual-compat-start -->
## Use When
- Build or improve a repeatable Geo Newsjacking workflow for a client or delivery team.
...
## Degraded Mode
If accounts, files, network, rendering or current evidence are unavailable, return the narrowest useful qualified Geo Newsjacking playbook plus an evidence-gap list. Mark each unavailable check `not assessed`; never convert it into a pass.

## Decision Rules
| Condition | Action | Failure or risk avoided |
|---|---|---|
| The event involves harm, uncertainty or political sensitivity | Pause and require editorial and risk approval | Exploiting a crisis or spreading falsehoods |
...
3. Apply the decision rules, preserve useful existing material and draft the Geo Newsjacking playbook.
...
|---|---|---|
| Geo Newsjacking playbook | Client owner and delivery team | Uses named inputs, assigns actions, states decisions and contains no unverified specifics |
