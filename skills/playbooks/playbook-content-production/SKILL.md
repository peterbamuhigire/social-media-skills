...
## Degraded Mode
If accounts, files, network, rendering or current evidence are unavailable, return the narrowest useful qualified Content Production playbook plus an evidence-gap list. Mark each unavailable check `not assessed`; never convert it into a pass.
...
3. Apply the decision rules, preserve useful existing material and draft the Content Production playbook.
...
|---|---|---|
| Content Production playbook | Client owner and delivery team | Uses named inputs, assigns actions, states decisions and contains no unverified specifics |
