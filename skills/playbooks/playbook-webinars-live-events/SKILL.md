...
description: Use when designing or improving a Webinars Live Events operating playbook with roles, ordered actions, controls and measures. Use platform skills for channel plans and strategy skills for upstream direction.
metadata:
  portable: true
  compatible_with:
  - claude-code
...
<!-- dual-compat-start -->
## Use When
- Build or improve a repeatable Webinars Live Events workflow for a client or delivery team.
...
## Degraded Mode
If accounts, files, network, rendering or current evidence are unavailable, return the narrowest useful qualified Webinars Live Events playbook plus an evidence-gap list. Mark each unavailable check `not assessed`; never convert it into a pass.

## Decision Rules
| Condition | Action | Failure or risk avoided |
|---|---|---|
| The live session lacks moderation, recording consent or failure backup | Do not go live until controls are assigned | Public harm or unusable event evidence |
...
3. Apply the decision rules, preserve useful existing material and draft the Webinars Live Events playbook.
...
|---|---|---|
| Webinars Live Events playbook | Client owner and delivery team | Uses named inputs, assigns actions, states decisions and contains no unverified specifics |
