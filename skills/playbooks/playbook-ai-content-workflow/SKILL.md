...
description: Use when designing or improving a Ai Content Workflow operating playbook with roles, ordered actions, controls and measures. Use platform skills for channel plans and strategy skills for upstream direction.
metadata:
  portable: true
  compatible_with:
  - claude-code
...
<!-- dual-compat-start -->
## Use When
- Build or improve a repeatable Ai Content Workflow workflow for a client or delivery team.
...
## Degraded Mode
If accounts, files, network, rendering or current evidence are unavailable, return the narrowest useful qualified Ai Content Workflow playbook plus an evidence-gap list. Mark each unavailable check `not assessed`; never convert it into a pass.
...
3. Apply the decision rules, preserve useful existing material and draft the Ai Content Workflow playbook.
...
|---|---|---|
| Ai Content Workflow playbook | Client owner and delivery team | Uses named inputs, assigns actions, states decisions and contains no unverified specifics |
