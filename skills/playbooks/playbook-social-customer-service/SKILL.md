---
name: playbook-social-customer-service
description: Use when designing or improving a Social Customer Service operating playbook with roles, ordered actions, controls and measures. Use platform skills for channel plans and strategy skills for upstream direction.
metadata:
  portable: true
  compatible_with:
  - claude-code
...
<!-- dual-compat-start -->
## Use When
- Build or improve a repeatable Social Customer Service workflow for a client or delivery team.
...
## Degraded Mode
If accounts, files, network, rendering or current evidence are unavailable, return the narrowest useful qualified Social Customer Service playbook plus an evidence-gap list. Mark each unavailable check `not assessed`; never convert it into a pass.

## Decision Rules
| Condition | Action | Failure or risk avoided |
|---|---|---|
| An issue needs personal or transaction data | Move to an approved private channel and minimise collection | Public disclosure of customer data |
...
3. Apply the decision rules, preserve useful existing material and draft the Social Customer Service playbook.
...
|---|---|---|
| Social Customer Service playbook | Client owner and delivery team | Uses named inputs, assigns actions, states decisions and contains no unverified specifics |
...
**1. Queries by Channel (Volume Tracker)**
Count incoming messages and comments by platform and by query type (see triage framework below). Identifies where volume is highest and where to focus training or automation.

**2. Speed of First Reply**
Average time from message received to first response sent. Most platforms (Facebook, Instagram) display this publicly as a response rate badge. Aim for the "Very responsive" badge on Facebook (90%+ of messages answered within 15 minutes — use auto-reply to achieve this).

**3. Resolution Rate**
Percentage of queries fully resolved without the customer needing to follow up. Track by logging outcomes: Resolved / Escalated / Unresolved / No response needed. Low resolution rate signals script gaps or staff knowledge deficits.
