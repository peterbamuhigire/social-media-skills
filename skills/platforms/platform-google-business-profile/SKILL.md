...
| Current platform rules and feature limits | Official Google Business Profile help or policy source | Conditional | Omit volatile specifications or flag them for live verification |
...
## Degraded Mode
If accounts, files, network, rendering or current evidence are unavailable, return the narrowest useful qualified Google Business Profile channel plan plus an evidence-gap list. Mark each unavailable check `not assessed`; never convert it into a pass.
...
3. Apply the decision rules, preserve useful existing material and draft the Google Business Profile channel plan.
...
|---|---|---|
| Google Business Profile channel plan | Client owner and delivery team | Uses named inputs, assigns actions, states decisions and contains no unverified specifics |
