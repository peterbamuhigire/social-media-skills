...
description: Use when creating a Instagram Growth channel plan covering account setup, content, community and measurement for Uganda or East Africa. Use a playbook for cross-channel operations and a strategy skill for channel selection.
metadata:
  portable: true
  compatible_with:
  - claude-code
...
<!-- dual-compat-start -->
## Use When
- Create or revise a Instagram Growth-specific presence, growth or publishing plan.
...
| Current platform rules and feature limits | Official Instagram Growth help or policy source | Conditional | Omit volatile specifications or flag them for live verification |
...
## Degraded Mode
If accounts, files, network, rendering or current evidence are unavailable, return the narrowest useful qualified Instagram Growth channel plan plus an evidence-gap list. Mark each unavailable check `not assessed`; never convert it into a pass.

## Decision Rules
| Condition | Action | Failure or risk avoided |
|---|---|---|
| Growth rises while saves, replies or enquiries fall | Reject follower volume as success and correct content or targeting | Vanity growth with no business value |
...
3. Apply the decision rules, preserve useful existing material and draft the Instagram Growth channel plan.
...
|---|---|---|
| Instagram Growth channel plan | Client owner and delivery team | Uses named inputs, assigns actions, states decisions and contains no unverified specifics |
...
1. **Shares to Stories or DMs** — highest reach signal; prioritise shareable content
2. **Saves** — indicate high-value content; use educational carousels, checklists, and how-to posts to drive saves
3. **Comments** — depth matters; a 5-word comment weighs less than a question or conversation thread
4. **Likes** — lowest weight; a post with many likes but few saves or shares will have limited reach
...
1. **Reels** — highest non-follower distribution; the primary discovery tool
2. **Carousels** — high dwell time; shown to followers more than once if they do not swipe through on first view
3. **Static images** — limited non-follower reach; use for grid cohesion and follower engagement, not discovery
4. **Stories** — seen only by existing followers; not a growth tool but critical for retention

> For a full breakdown of Instagram algorithm ranking signals and the pre-publication checklist, see `meta-algorithm-guide`.
...
- Include the primary keyword in the **username** (e.g. `@kampala.skincare` not `@beautybykamini`)
- Include the primary keyword and location in the **display name** (e.g. "Kampala Skincare | Natural Products")
...
1. Search the niche keyword on Instagram (e.g. "Kampala food")
2. Note the top 5–10 hashtags used by the most-engaged posts (not the most-followed accounts)
3. Check each hashtag's post volume — build a shortlist of 20–30 tags across all three tiers
4. Rotate between 3–4 different hashtag sets across posts to avoid appearing spammy
5. Audit hashtag performance monthly in Instagram Insights (reach from hashtags metric)
...
**Diagnostic steps:**
1. Audit the last 20 posts for content consistency and niche focus
2. Check for any posts that may have triggered a restriction (prohibited content, over-tagging, mass following activity)
3. Review hashtag performance in Insights and rotate the hashtag sets
4. Publish 3–5 Reels in the following 2 weeks and monitor reach recovery
5. Cross-reference with `meta-algorithm-guide` for the current Instagram algorithm diagnostic checklist
