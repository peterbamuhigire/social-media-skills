...
description: >
  Systematic process for mining customer questions from five sources and
  converting them into a content pipeline of blog posts, social posts, video
  scripts, and FAQs that rank organically and answer what the audience is
  already searching for. Invoke when a client has no content strategy, when
  content ideas have run dry, when the client asks what to post, or when
  the goal is to generate content that reduces sales friction by pre-educating
  prospects before they speak to the business.
---
